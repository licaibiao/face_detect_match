...
void saveImage1(const char *filename, int Width, int Height, int Channels, unsigned char *Output) {
	//淇濆瓨涓簀pg
...
void CropImg(unsigned char *sourceData, int width, int height, int Channels,
	unsigned char *destinationData,int left,int top, int newWidth, int newHeight) {
	if (sourceData == NULL || destinationData == NULL) return;
	int rowWidth = width * Channels;
	unsigned char *p2 = destinationData;
	for (int y = top; y < height &&y < newHeight + top; y++) {
		unsigned char *p1 = sourceData + y * rowWidth + left * Channels;
		for (int x = left; x < width && x < newWidth + left; x++) {
...
/******************************************************** 
Function:	 MTCNN_InitOutFileName	
Description: 命名输出文件
Input:	outFile
OutPut: outFile
...
Date:	2018-10-25
*********************************************************/
int MTCNN_InitOutFileName(MTCNN_OUT_IMAGE_S *outFile)
{
	if(NULL==outFile)
	{
	...
	{
		sprintf(outFile->FileName[i],"%s/%ld_%d.jpg",outFile->PathName,now,i);
	}
...
int MTCNN_FaceNumDetect(char * ps8ImagePath, unsigned char *pu8FaceNum)
{
	...
	int minSize = 32;// atoi(argv[3]);
	float minSizeScale = 0.1;// 

	getCurrentFilePath(ps8ImagePath, saveFile);
	unsigned char *inputImage = loadImage(ps8ImagePath, &Width, &Height, &Channels);

	if (inputImage == nullptr || Channels != 3)
	{
		printf("%s %d load image error \n",__FUNCTION__,__LINE__);
		return -1;
	}

	ncnn::Mat ncnn_img = ncnn::Mat::from_pixels(inputImage, ncnn::Mat::PIXEL_RGB, Width, Height);
	std::vector<Bbox> finalBbox;
	MTCNN mtcnn(MTCNN_MODEL_PATH_NAME,minSize);
...
int MTCNN_FaceCutOut(char *ps8ImagePath, MTCNN_OUT_IMAGE_S *OutImge)
{
	...
	MTCNN_InitOutFileName(OutImge);

	if(num_box>MAX_FACE_NUM)
	{
		num_box = MAX_FACE_NUM;
	}

	OutImge->FaceNum = num_box;

    for (int i = 0; i < num_box; i++) 
	{
		int cw = finalBbox[i].x2 - finalBbox[i].x1;
		int cy = finalBbox[i].y2 - finalBbox[i].y1;
		unsigned char *newImg = new unsigned char[cy*cw*Channels];
		CropImg(inputImage, Width, Height, Channels, newImg, finalBbox[i].x1, finalBbox[i].y1, cw, cy);

		saveImage1(OutImge->FileName[i], cw, cy, Channels, newImg);
		if(NULL!=newImg)
		{
			delete []newImg;
		}
...
#if defined(__FOR_ARM_DEBUG__)
int main(void)
{
	//MTCNN_FaceNumDetect("./imag/sample.jpg");

	MTCNN_OUT_IMAGE_S stOutInfo = {0};
	char arru8Path[64] = "./out";

	/**设置输出目录**/
	strcpy(stOutInfo.PathName,arru8Path);

	MTCNN_FaceCutOut("./imag/sample.jpg",&stOutInfo);

	printf("face num = %d \n",stOutInfo.FaceNum);
	for(int i=0;i<stOutInfo.FaceNum;i++)
	{
		printf("Image %d name = %s \n",i,stOutInfo.FileName[i]);
	}
...
#if defined(__FOR_UBUNTU_DEBUG_)
int main(int argc, char **argv) {
	...
	if (argc > 4) {
		if (Height < Width)
			minSize = Height * minSizeScale;
		else
			minSize = Width * minSizeScale;
	}
	ncnn::Mat ncnn_img = ncnn::Mat::from_pixels(inputImage, ncnn::Mat::PIXEL_RGB, Width, Height);
	std::vector<Bbox> finalBbox;
	MTCNN mtcnn(model_path,minSize);
	double startTime = now();
	mtcnn.detect(ncnn_img, finalBbox);
	double nDetectTime = calcElapsed(startTime, now());
	printf("time: %d ms.\n ", (int)(nDetectTime * 1000));
	int num_box = finalBbox.size();
	printf("face num: %u \n", num_box);
	bool draw_face_feat = true;// false;
    int left_eye_x = 0;
    int left_eye_y = 0;
    int right_eye_x = 0;
    int right_eye_y = 0;
    for (int i = 0; i < num_box; i++) {
        if (draw_face_feat) {
            ...
            drawRectangle(inputImage, Width, Channels, finalBbox[i].x1, finalBbox[i].y1,
                          finalBbox[i].x2,
                          finalBbox[i].y2, red);
            const uint8_t blue[3] = {0, 255, 255};

            for (int num = 0; num < 5; num++) {
				int x1 = (int)(finalBbox[i].ppoint[num] + 0.5f);
				int y1 = (int)(finalBbox[i].ppoint[num+5] + 0.5f);
                drawPoint(inputImage, Width, Channels, (int) (finalBbox[i].ppoint[num] + 0.5f),
                          (int) (finalBbox[i].ppoint[num + 5] + 0.5f), blue);

				drawRectangle(inputImage, Width, Channels, x1-1, y1-1,
					x1+1,
					y1+1, blue);
            }
			int cw = finalBbox[i].x2 - finalBbox[i].x1;
			...
			CropImg(inputImage, Width, Height, Channels, newImg, finalBbox[i].x1, finalBbox[i].y1, cw, cy);
			char newName[128];
			sprintf(newName, "FaceImage%d.jpg", i);
			saveImage1(newName, cw, cy, Channels, newImg);
			delete newImg;
        }
